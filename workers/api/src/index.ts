...
export async function seoForPath(pathname: string, env?: Pick<Env, "DB" | "FACTORY_ADDRESS">): Promise<SeoMetadata> {
  ...
  if (pathname.startsWith("/what-does-economic-allocation-consistency-mean")) {
    return { title: "What Does Economic-Allocation Consistency Mean? | Rugspull", description: "Learn how Founder, Opening, and initial pool allocations partition RugToken supply—and why these recorded fields are not live balances.", robots: "index, follow", canonical };
  }
  if (pathname.startsWith("/why-opening-price-is-not-below-initial-pool-price")) {
    return { title: "Why Is the Opening Price Not Below the Initial Pool Price? | Rugspull", description: "See how Rugspull derives the Opening and initial canonical-pool prices, why integer rounding favors the pool side, and what the inequality does not guarantee.", robots: "index, follow", canonical };
  }
  if (pathname.startsWith("/why-pro-rata-claims-can-leave-rounding-residue")) {
    return { title: "Why Can Pro-Rata Claims Leave Rounding Residue? | Rugspull", description: "See how per-wallet floor division allocates successful Opening tokens and excess WBNB, why aggregate payouts cannot exceed their pools, and where residue remains.", robots: "index, follow", canonical };
  }
  if (pathname.startsWith("/what-does-settled-mean-for-claims-and-refunds")) {
    return { title: "What Does Settled Mean for Claims and Refunds? | Rugspull", description: "Learn when Rugspull reporting may call contributor claims or Failed refunds settled, which receipts and flags prove completion, and why settled does not mean a zero contract balance.", robots: "index, follow", canonical };
  }
  if (pathname.startsWith("/how-to-verify-claims-and-refunds-on-bscscan")) {
    return { title: "How to Verify Claims and Refunds on BscScan | Rugspull", description: "Follow RugInstance contribution, claim, refund, and Creator-withdrawal events; confirm receipts and flags; and reconcile amounts at a named BNB Chain cutoff.", robots: "index, follow", canonical };
  }
  if (pathname.startsWith("/what-is-a-transaction-receipt-on-bnb-chain")) {
    return { title: "What Is a Transaction Receipt on BNB Chain? | Rugspull", description: "Learn what a BNB Chain transaction receipt proves, how success differs from submission or simulation, and how logs, gas, transfers, and current state fit together.", robots: "index, follow", canonical };
  }
