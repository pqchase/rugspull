...
DEPLOYMENT_FILE="${DEPLOYMENT_FILE:-$ROOT/deployments/bsc-mainnet.json}"
FACTORY_ADDRESS="${FACTORY_ADDRESS:-$(jq -r '.factory' "$DEPLOYMENT_FILE")}"
RPC_URL="${RPC_URL:-https://bsc.rpc.blxrbdn.com}"
VERIFY_DRY_RUN="${VERIFY_DRY_RUN:-0}"
API_KEY="${BSCSCAN_API_KEY:-${ETHERSCAN_API_KEY:-}}"

if [ "$VERIFY_DRY_RUN" != "1" ] && [ -z "$API_KEY" ]; then
  echo "BSCSCAN_API_KEY or ETHERSCAN_API_KEY is required unless VERIFY_DRY_RUN=1." >&2
  exit 1
fi
...
if [ "$VERIFY_DRY_RUN" = "1" ]; then
  echo "Would verify $FACTORY_ADDRESS as contracts/src/RugFactory.sol:RugFactory on BSC Mainnet."
