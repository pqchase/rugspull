...
const EVERGREEN_ROUTE_LABELS = {
  ...
  "/what-does-economic-allocation-consistency-mean": "What does economic-allocation consistency mean?",
  "/why-opening-price-is-not-below-initial-pool-price": "Why is the Opening price not below the initial pool price?",
  "/why-pro-rata-claims-can-leave-rounding-residue": "Why can pro-rata claims leave rounding residue?",
                ...
                : path.startsWith("/transparency")
                  ...
                  : path.startsWith("/security-model")
                    ...
                    : path.startsWith("/what-is-a-crypto-rug-pull")
                      ...
                      : path.startsWith("/what-does-token-conservation-mean")
                        ? { title: "What Does Token Conservation Mean? | Rugspull", description: "Trace RugToken's fixed supply across RugInstance, Opening claims, RugPool swaps, and the one-shot Founder sale—and learn what conservation does not prove.", robots: "index, follow" }
                      : path.startsWith("/what-does-wbnb-conservation-mean")
                        ? { title: "What Does WBNB Conservation Mean? | Rugspull", description: "Trace WBNB through creation fees, Creator stake, contributions, refunds, RugPool swaps, protocol fees, and the Founder sale—and learn what conservation does not prove.", robots: "index, follow" }
                      : path.startsWith("/what-does-no-double-claim-mean")
                        ? { title: "What Does No Double Claim Mean? | Rugspull", description: "Learn how Rugspull makes successful Opening claims and Failed refunds one-shot per contributing wallet—and what that protection does not prove.", robots: "index, follow" }
                      : path.startsWith("/what-does-no-double-rug-mean")
                        ? { title: "What Does No Double Rug Mean? | Rugspull", description: "Learn why Rugspull's protocol-held Founder Allocation can be sold only once in full—and what that one-shot rule does not protect.", robots: "index, follow" }
                      : path.startsWith("/what-does-reserve-reconciliation-mean")
                        ? { title: "What Does Reserve Reconciliation Mean? | Rugspull", description: "Learn how RugPool stored token and WBNB reserves match canonical swap balances, why direct transfers create surplus, and how to verify the difference.", robots: "index, follow" }
                      : path.startsWith("/what-does-founder-token-immobility-mean")
                        ? { title: "What Does Founder Token Immobility Mean? | Rugspull", description: "Learn why Rugspull keeps the Founder Allocation in RugInstance until its one-shot sale, why unlock does not transfer tokens, and what the custody invariant does not prove.", robots: "index, follow" }
                      : path.startsWith("/what-does-protocol-fee-destination-mean")
                        ? { title: "What Does Protocol Fee Destination Mean? | Rugspull", description: "Learn how Rugspull routes creation and swap protocol fees in WBNB, why the pool fee stays in RugPool, and what the destination invariant actually covers.", robots: "index, follow" }
                      : path.startsWith("/what-does-non-decreasing-amm-k-mean")
                        ? { title: "What Does Non-Decreasing AMM K Mean? | Rugspull", description: "Learn why RugPool's stored reserve product should not decrease across canonical swaps, how fees and rounding affect k, and what the invariant does not prove.", robots: "index, follow" }
                      : path.startsWith("/what-does-status-monotonicity-mean")
                        ? { title: "What Does Lifecycle-Status Monotonicity Mean? | Rugspull", description: "Learn why a RugInstance can move from Opening to Failed or Active, and from Active to Rugged, but cannot reopen or move backward.", robots: "index, follow" }
                      : path.startsWith("/what-does-economic-allocation-consistency-mean")
                        ? { title: "What Does Economic-Allocation Consistency Mean? | Rugspull", description: "Learn how Founder, Opening, and initial pool allocations partition RugToken supply—and why these recorded fields are not live balances.", robots: "index, follow" }
                      : path.startsWith("/why-opening-price-is-not-below-initial-pool-price")
                        ? { title: "Why Is the Opening Price Not Below the Initial Pool Price? | Rugspull", description: "See how Rugspull derives the Opening and initial canonical-pool prices, why integer rounding favors the pool side, and what the inequality does not guarantee.", robots: "index, follow" }
                      : path.startsWith("/why-pro-rata-claims-can-leave-rounding-residue")
                        ? { title: "Why Can Pro-Rata Claims Leave Rounding Residue? | Rugspull", description: "See how per-wallet floor division allocates successful Opening tokens and excess WBNB, why aggregate payouts cannot exceed their pools, and where residue remains.", robots: "index, follow" }
                      : path.startsWith("/what-does-settled-mean-for-claims-and-refunds")
                        ? { title: "What Does Settled Mean for Claims and Refunds? | Rugspull", description: "Learn when Rugspull reporting may call contributor claims or Failed refunds settled, which receipts and flags prove completion, and why settled does not mean a zero contract balance.", robots: "index, follow" }
                      : path.startsWith("/how-to-verify-claims-and-refunds-on-bscscan")
...
const FACT_PAGES = {
  ...
  "/what-does-token-conservation-mean": {
    eyebrow: "Builder desk · invariant 01 · token conservation",
    title: "TOKENS MOVE BETWEEN ACCOUNTS. THE SUPPLY DOES NOT APPEAR OR VANISH.",
    intro: "Token conservation asks whether every RugToken unit remains accounted for as calls move balances among RugInstance, RugPool, contributors, and other holders. It is a supply-accounting property—not proof of fair distribution, useful liquidity, stable price, or safety.",
    sections: [
      ["Failed Opening creates no RugToken", "RugInstance is deployed before a token exists. If total contribution misses the immutable minimum, finalize records Failed and returns without creating RugToken or RugPool. Token conservation therefore begins only on a successful launch; a Failed lifecycle has WBNB refund accounting but no RugToken supply to sum."],
      ["Successful finalization creates the fixed supply once", "RugToken's constructor records immutable totalSupply, assigns the entire amount to RugInstance as initialHolder, and emits the sole mint-like Transfer from the zero address. The contract exposes transfer, approve, and transferFrom, but no later mint or burn function. Names and symbols do not determine supply; the deployed token address and totalSupply do."],
      ["The initial supply is divided by formulas", "On success, founder allocation f is derived from tokenTotalSupply and founderBps. Non-Founder supply n equals totalSupply minus f; Opening allocation a is calculated from n, Creator stake, and accepted contribution; pool token reserve x equals n minus a. Before claims, RugInstance therefore holds f plus a while RugPool receives x."],
      ["The 45% Founder Allocation stays in RugInstance", "For the deployed v0.4 configuration, f is 45% of supply. It is not transferred to the Creator wallet at launch or unlock. founderRemaining is an accounting value for the protocol-held amount, while the actual tokens remain in RugInstance until the one-shot Founder sale. The rule does not identify related wallets or ordinary tokens the Creator later buys."],
      ["Opening claims transfer existing tokens", "After Active or Rugged, each contributor calls claimOpening from the contributing wallet. The contract calculates floor(openingTokenAllocation × walletContribution ÷ totalContributed), marks the wallet claimed, and transfers that amount out of RugInstance. Rounding can leave residual units in RugInstance; it does not mint extra units or let aggregate claims exceed the allocation."],
      ["Canonical swaps move tokens between holder and pool", "A buy transfers WBNB input and sends existing RugToken from RugPool to the recipient. A sell transfers existing RugToken into RugPool and sends WBNB output. Approvals authorize transferFrom but do not create supply. Direct transfers to RugPool can create balance surplus above stored reserves, so conservation and reserve reconciliation are related but distinct checks."],
      ["The Founder sale moves the full remaining amount once", "After unlock, Creator-only rug() sets founderRemaining to zero, changes status to Rugged, transfers the entire protocol-held amount from RugInstance to RugPool, and executes a token-for-WBNB sale. The Creator receives WBNB output rather than Founder Tokens. If the call reverts, EVM atomicity rolls back those state and balance changes."],
      ["A complete accounting needs every holder in scope", "At any block after token creation, the conceptual check is the sum of balanceOf across every address equals immutable totalSupply. The project invariant uses a closed model: RugInstance, RugPool, Creator, treasury, handler, and three actors. Production has an unbounded address set, so an indexer must reconstruct transfers or query known balances without pretending a partial holder list is exhaustive."],
      ["Conservation does not mean the economic labels stay spendable", "After claims or the Founder sale, openingTokenAllocation and poolTokenReserve remain launch facts rather than live wallet balances. founderRemaining changes from the full allocation to zero. Adding historical allocation fields together after tokens move can double-count; current balanceOf, Transfer history, lifecycle state, and stored reserves answer different questions."],
      ["A fixed sum is not a safety verdict", "Token conservation can catch accidental minting, burning, omission, or accounting drift inside a stated model. It does not prove fair ownership, anti-sybil protection, adequate WBNB reserves, acceptable price, MEV protection, correct metadata, key security, a buyer, an exit, or recovery. The property and harness are project-authored, not exhaustive or independent. Audit and organized mainnet activation remain pending; total loss remains possible."],
    ],
  },
  "/what-does-wbnb-conservation-mean": {
    eyebrow: "Builder desk · invariant 02 · WBNB conservation",
    title: "WBNB CHANGES HOLDERS. ACCOUNTING MUST NOT LOSE A WEI.",
    intro: "WBNB conservation asks whether the modeled wrapped-BNB supply remains fully accounted for as stake, contributions, refunds, pool inventory, trade proceeds, and fees move among contracts and wallets. A conserved total does not mean every unit is liquid, refundable, recoverable, or safe.",
    sections: [
      ["Conservation starts with an exact asset and scope", "The property concerns the canonical WBNB token balanceOf and totalSupply, not native BNB used for gas, another wrapped token, a stablecoin, or a database total. Every address included in the model must use the same WBNB contract. A complete production reconciliation also needs the exact chain, block, contracts, and holder scope."],
      ["Creation separates the fee from Creator stake", "RugFactory transfers creatorStake plus creationFee from the Creator. It sends the creation fee immediately to the immutable protocol treasury, deploys RugInstance, and transfers the stake to that instance. For deployed v0.4, the fee is 0.003 WBNB and the minimum stake is 0.1 WBNB. The fee is spent at creation and is not a refundable stake balance."],
      ["Opening contributions accumulate in RugInstance", "Each non-Creator contribution transfers WBNB from the contributing wallet into RugInstance and increments that wallet's contribution plus totalContributed. No pool exists yet, and contribution is not a swap. The recorded amount is an entitlement input; current WBNB balance and the later lifecycle result determine where the held units can move next."],
      ["Failed returns contribution and stake through separate claims", "If total contribution is below the minimum, Failed creates no token or pool. Each contributor must call claimFailedRefund for that wallet's full recorded contribution, while the Creator separately calls withdrawCreatorStakeAfterFailure. Both paths are one-shot. The creation fee stays in treasury, and native-BNB gas is outside WBNB refund accounting."],
      ["Success moves stake plus accepted contribution into RugPool", "On a successful finalization, accepted contribution q is capped by the immutable Opening cap and initial poolQuoteReserve y equals creatorStake plus q. RugInstance transfers y WBNB into RugPool. If total contribution exceeds q, the difference remains in RugInstance for proportional excess-refund claims rather than becoming canonical pool reserve."],
      ["Successful claims can leave rounding residue", "claimOpening calculates floor((totalContributed - acceptedContribution) × walletContribution ÷ totalContributed) and transfers that WBNB refund from RugInstance. Integer rounding can make aggregate refunds slightly smaller than the nominal excess, leaving residual WBNB in RugInstance. The units are conserved but may be stranded because no general surplus-withdraw function exists."],
      ["A buy splits WBNB input between pool and treasury", "For a canonical buy, RugPool pulls the submitted WBNB from the buyer. The protocol portion is calculated from quoteIn and transferred to treasury; the remainder becomes poolQuoteIn and increases stored quote reserve. The retained 0.25% pool fee is reflected inside swap arithmetic, while the 0.05% protocol portion changes holder without disappearing."],
      ["A sell splits gross WBNB output between recipient and treasury", "For an ordinary token sell or the full Founder sale, RugPool calculates grossQuoteOut, reduces stored quote reserve by that gross amount, transfers net quoteOut to the recipient, and transfers the protocol fee to treasury. Founder rug() sends its net WBNB output to the recorded Creator. A revert atomically rolls back transfers and reserve changes, although native-BNB gas can still be spent."],
      ["The invariant model is closed and narrower than Factory E2E", "invariant_WbnbIsConserved sums MockWBNB balances held by RugInstance, RugPool when present, Creator, treasury, handler, and three actors, then compares that sum with MockWBNB totalSupply. Its handler starts from an already constructed and funded RugInstance, so it does not exercise RugFactory's creation-fee split; Factory unit and scenario tests cover that separate path."],
      ["Conserved WBNB is not guaranteed recoverable WBNB", "This property can catch modeled WBNB appearing, disappearing, or being omitted from a closed balance sum. It does not prove that every holder was included, funds are withdrawable, refund rounding leaves no residue, treasury custody is safe, reserves are deep, price is acceptable, transactions will execute, or losses can be recovered. The harness is project-authored and bounded, not an audit or safety certification. Organized mainnet activation remains pending; total loss remains possible."],
    ],
  },
  "/what-does-no-double-claim-mean": {
    eyebrow: "Builder desk · invariant 03 · no double claim",
    title: "ONE WALLET. ONE LIFECYCLE CLAIM. NO SECOND PAYOUT.",
    intro: "Rugspull aggregates a wallet's Opening contributions and exposes one lifecycle claim after finalization. The contract records completion before transferring assets, rejects a second successful call, and rolls everything back if the first attempt reverts. That is replay protection—not proof that everyone has claimed or can recover every loss.",
    sections: [
      ["Contributions aggregate by wallet", "contributionOf adds every successful Opening contribution made by the same address. Finalization does not create one claim per transaction. The contributing wallet receives one aggregate entitlement based on its cumulative amount, so splitting that wallet's contribution into several calls does not create several claim rights."],
      ["The lifecycle chooses exactly one contributor claim path", "After a successful finalization, status is Active and later may become Rugged; the wallet uses claimOpening. After a failed finalization, status is Failed; the wallet uses claimFailedRefund. BadStatus rejects the wrong path. A finalized Rug cannot switch between Failed and Active, so the two contributor claim functions are mutually exclusive for that lifecycle."],
      ["One claimed flag covers the contributing wallet", "Both contributor claim functions check claimed[msg.sender] and revert with AlreadyClaimed when it is already true. They also require a nonzero contribution and otherwise revert with NothingToClaim. The flag is scoped to one RugInstance and one wallet; it is not a global identity, anti-sybil control, or proof that related wallets do not exist."],
      ["Successful Opening claim can transfer two assets once", "claimOpening calculates the wallet's proportional RugToken amount and any excess-WBNB refund, sets claimed true, then transfers nonzero amounts and emits ClaimedOpening. Token and refund are one atomic claim operation. A wallet cannot claim tokens first and request the excess refund later as a second contract call."],
      ["Failed claim returns recorded contribution once", "claimFailedRefund reads the wallet's full recorded contribution, sets claimed true, transfers that WBNB, and emits ClaimedFailedRefund. It does not return the Creator's stake, the creation fee, native-BNB gas, trading losses, or assets held by another wallet. Each contributing address must call for itself."],
      ["State is written before external transfers", "Both contributor paths follow checks-effects-interactions: validate status and entitlement, write claimed true, then call the token contract. RugInstance also applies a nonReentrant lock. This ordering blocks a token callback from re-entering the same instance to collect the entitlement again before the first transfer finishes."],
      ["A reverted first attempt does not burn the entitlement", "EVM atomicity rolls back the claimed flag, token transfers, WBNB transfers, and event if any later step reverts. The wallet may retry after fixing the cause, such as RPC, gas, or transaction parameters. A failed transaction receipt is therefore not a completed claim and must not be counted as settled."],
      ["Creator stake withdrawal is a separate one-shot path", "On Failed, the recorded Creator calls withdrawCreatorStakeAfterFailure. It uses creatorStakeWithdrawn rather than the contributor claimed mapping, checks the exact Creator, and rejects a second withdrawal with AlreadyWithdrawn. The double-claim invariant probe does not exercise this flag; dedicated unit and scenario tests cover it separately."],
      ["The invariant deliberately retries completed contributor claims", "After a modeled actor becomes claimed, probeDoubleClaim calls claimFailedRefund in Failed or claimOpening otherwise using a low-level call. Any successful retry sets doubleClaimSucceeded true, and invariant_ClaimsAndRugCannotExecuteTwice requires that flag to remain false. In the reported run, the combined property completed 128 runs and 8,192 handler calls with zero handler reverts."],
      ["No double claim does not mean fully settled", "A one-shot guard does not push assets automatically, prove every eligible wallet claimed, eliminate rounding residue, restore lost keys, pay gas, guarantee frontend or RPC availability, or recover losses from swaps or the Founder sale. It also does not establish exhaustive security: the harness uses selected actors, actions, mocks, and bounded sequences. Project-authored tests are not an independent audit or safety certification. Organized mainnet activation remains pending; total loss remains possible."],
    ],
  },
  "/what-does-no-double-rug-mean": {
    eyebrow: "Builder desk · invariant 04 · no double rug",
    title: "ONE FOUNDER ALLOCATION. ONE FULL SALE. NO SECOND RUG CALL.",
    intro: "Rugspull deliberately permits one Creator-triggered sale of the protocol-held Founder Allocation after unlock. A successful call consumes the entire allocation and permanently advances the lifecycle to Rugged, so the same protocol allocation cannot be sold again. This is a one-shot accounting rule—not protection from the first sale or from ordinary wallet activity.",
    sections: [
      ["The rule applies to one RugInstance", "Each successful RugInstance records one immutable Creator, one Founder Allocation formula, one canonical RugPool, and one founderUnlockTime. The no-double-rug property is scoped to that contract instance and its protocol-held Founder Tokens. It is not a global identity rule across several Rugs, Creators, wallets, tokens, or external pools."],
      ["Only the recorded Creator may call rug()", "rug() first requires msg.sender to equal the immutable Creator and otherwise reverts with NotCreator. Delegates, contributors, the Factory owner, treasury, frontend, Worker, and arbitrary callers cannot execute this protocol path. Creator authorization does not prove the Creator key is secure or that related wallets are absent."],
      ["The lifecycle must still be Active", "The function accepts only status Active. Opening, Failed, and already Rugged instances revert with BadStatus. A successful Founder sale changes status to Rugged, and lifecycle status never returns to Active, so the same entry condition cannot become true again for that instance."],
      ["The full post-Opening lock must expire", "Even while Active, block.timestamp must be at least founderUnlockTime or the call reverts with FounderLocked. For deployed v0.4, the Founder Allocation stays in RugInstance throughout the 24-hour Opening and a further 48-hour lock. The unlock is permission to attempt the one sale, not a deadline, promise, or automatic transaction."],
      ["Partial Founder sales are not an option", "rug() reads the entire founderRemaining amount and exposes no amount parameter for the Creator to choose. The deployed configuration starts with 45% of fixed RugToken supply as the protocol-held allocation. Ordinary RugToken acquired by the Creator or related wallets is outside this accounting field and can move through ordinary token or pool functions."],
      ["Effects consume the right before external calls", "After its checks, rug() requires founderRemaining to be nonzero, writes founderRemaining to zero, changes status to Rugged, and only then transfers tokens to RugPool and calls sellFromRugInstance. The function is nonReentrant. This checks-effects-interactions ordering prevents a callback from observing an unchanged Founder balance and executing the protocol sale twice."],
      ["A revert rolls back the attempted consumption", "The token transfer, RugPool sale, slippage minimum, recipient, deadline, reserve updates, protocol fee, lifecycle status, and founderRemaining write are one EVM transaction. If a later step reverts, all state and token movements roll back. The Creator may retry a failed attempt; retrying a reverted transaction is not a second successful rug."],
      ["A successful sale leaves three independent traces", "After success, status is Rugged, founderRemaining is zero, and RugPulled records the full token amount plus net WBNB output. The Founder Tokens enter canonical pool reserves and the Creator receives WBNB after fees; the Creator wallet does not receive the Founder Tokens. Verify all three traces and the transaction receipt rather than infer completion from a label or database row."],
      ["The invariant deliberately retries after Rugged", "Once the modeled instance is Rugged, probeDoubleRug impersonates the recorded Creator and low-level calls rug() again. Any successful retry sets doubleRugSucceeded true. invariant_ClaimsAndRugCannotExecuteTwice requires that flag to remain false; in the reported combined run, the property completed 128 runs and 8,192 handler calls with zero handler reverts."],
      ["No double rug does not mean no rug risk", "The rule intentionally allows the first full Founder sale and does not cap its price impact, guarantee WBNB output, preserve price, create a refund, identify related wallets, stop ordinary token sales, police alternative pools, restore lost keys, or guarantee an exit. Claims and canonical trading can continue after Rugged. The bounded project-authored harness is not exhaustive, an independent audit, or a safety certification. Organized mainnet activation remains pending; total loss remains possible."],
    ],
  },
  "/what-does-reserve-reconciliation-mean": {
    eyebrow: "Builder desk · invariant 05 · reserve reconciliation",
    title: "TWO RESERVES. TWO BALANCES. RECONCILE THE DIFFERENCE.",
    intro: "Reserve reconciliation compares RugPool's stored token and WBNB reserves with the actual token balances held by the canonical pool. Canonical initialization and swaps are designed to keep them equal, while unsolicited direct transfers can create visible surplus. Equality is an accounting property inside a stated scope—not proof of liquidity, price, exit, or safety.",
    sections: [
      ["Start with the exact canonical contracts", "Identify chain id, RugInstance, its recorded pool, its recorded RugToken, and the pool's immutable WBNB. reserveToken must be compared with that RugToken's balanceOf(pool), and reserveQuote with that WBNB's balanceOf(pool), at the same block. A similarly named token, an alternative pool, native BNB, an indexer row, or an earlier block is a different scope."],
      ["Successful finalization establishes the first pair", "A Failed Opening creates no pool. On success, RugInstance transfers the calculated pool token reserve and Creator stake plus accepted contribution to the new RugPool, then initialize records those exact launch amounts once. AlreadyInitialized blocks a second initialization, while uint112 overflow checks prevent stored reserves from silently truncating."],
      ["A canonical buy updates both sides coherently", "For buyExactQuoteForTokens, protocolFeeQuote is separated from quoteIn and sent to treasury. Stored quote reserve increases only by poolQuoteIn, while stored token reserve decreases by tokensOut. The pool then receives the full quote input, pays the protocol portion onward, and sends tokens out, so its net balance changes match the new stored reserves."],
      ["An ordinary sell reconciles gross WBNB output", "sellExactTokensForQuote first transfers tokenIn into RugPool. The pool adds that amount to stored token reserve and subtracts grossQuoteOut from stored quote reserve. It then pays quoteOut to the recipient and protocolFeeQuote to treasury; those two transfers sum to grossQuoteOut, so the WBNB balance falls by the same amount as the stored reserve."],
      ["The Founder sale uses the same sell accounting", "RugInstance transfers the entire founderRemaining token amount to RugPool before calling sellFromRugInstance. That pool path is Creator-inaccessible except through the recorded instance, adds tokenIn to stored reserve, and splits gross WBNB output between the Creator and treasury. A successful one-shot Founder sale should therefore reconcile like another canonical sell."],
      ["Atomic reverts protect the pair from partial updates", "RugPool writes new reserves before some external token transfers and uses a nonReentrant lock, but EVM atomicity rolls back reserve writes, transfers, and events if any later operation reverts. A failed transaction receipt is not a new reserve checkpoint. Verification should use the last successful confirmed state, not simulated output or pending calldata."],
      ["The invariant covers modeled canonical actions", "invariant_CanonicalSwapsKeepReservesEqualToBalances checks actual RugToken and WBNB balances against getReserves whenever a pool exists. The handler explores lifecycle actions, buys, sells, and the Founder sale with selected actors and bounded inputs. In the reported configuration the property runs 128 sequences at depth 64, or 8,192 targeted calls; that is substantial but not exhaustive."],
      ["Direct transfers intentionally appear as surplus", "Anyone can transfer RugToken or WBNB directly to the pool address without calling a swap. RugPool has no sync or skim function, so those units do not change stored reserves or canonical quote arithmetic. The dedicated unit test sends 10 tokens and 0.25 WBNB directly, then confirms actual balances exceed stored reserves by exactly those amounts. In that case inequality identifies surplus rather than canonical accounting drift."],
      ["Verify equality or quantify surplus at one block", "Read getReserves, token(), WBNB(), and initialized from the canonical pool; then call balanceOf(pool) on both token contracts at the same block. Calculate token surplus as actual token balance minus reserveToken and quote surplus as actual WBNB balance minus reserveQuote. Negative differences contradict the expected model and require transaction, event, bytecode, and token-behavior investigation; positive differences can be traced as direct-transfer surplus."],
      ["Reconciliation is not a safety or liquidity verdict", "Matching values can catch canonical bookkeeping regressions, while a quantified surplus makes outside transfers explicit. Neither result proves the token is non-rebasing, every integration is correct, k never decreases outside scope, reserves are deep, price is fair, ordering is benign, a buyer or exit exists, keys are safe, or losses are recoverable. These tests are project-authored; independent audit and organized mainnet activation remain pending, and total loss remains possible."],
    ],
  },
  "/what-does-founder-token-immobility-mean": {
    eyebrow: "Builder desk · invariant 06 · Founder Token immobility",
    title: "THE FOUNDER ALLOCATION STAYS IN RUGINSTANCE UNTIL THE ONE-SHOT SALE.",
    intro: "Founder Token immobility means the protocol does not transfer the Founder Allocation into the Creator wallet at launch, during Opening, or when the lock expires. The allocation remains in RugInstance until the contract moves the full remaining amount directly into RugPool for the one-shot sale. This custody rule is narrower than a claim that the Creator can never own ordinary RugToken.",
    sections: [
      ["Founder Token is an allocation label, not a separate token class", "RugToken is fungible: units do not carry a permanent Founder, Opening, or pool label after transfer. Founder Allocation describes the formula-derived amount initially retained for the protocol sale. Verification must therefore combine RugInstance state, token balances, lifecycle events, and transaction paths instead of trying to identify special token IDs."],
      ["A Failed Opening creates no Founder Tokens", "RugInstance exists during Opening, but RugToken does not. If contribution misses the immutable minimum, finalize sets Failed and returns without deploying a token or pool and without calculating founderRemaining. Failed contributors claim WBNB refunds and the Creator can withdraw the failed stake; there is no Founder Allocation to transfer."],
      ["Successful finalization mints the entire supply to RugInstance", "RugToken's constructor mints the fixed totalSupply once to RugInstance, not to the Creator. RugInstance calculates founder allocation f, Opening allocation a, and pool reserve x; it sends x to RugPool and retains f plus a for protocol custody and contributor claims. In deployed v0.4, f is 45% of the fixed supply."],
      ["founderRemaining records custody intent", "On successful finalization, founderRemaining is set to f while the corresponding fungible tokens remain inside RugInstance. The variable is not a second token balance and does not mint or escrow a distinct asset. Opening claims reduce RugInstance's actual balance by transferring contributor entitlements, while founderRemaining stays unchanged until the Founder sale."],
      ["RugInstance exposes no Creator withdrawal path for the allocation", "The public contract interface has contribution, finalization, claims, Failed-stake withdrawal, and rug functions. It exposes no function that transfers Founder Tokens to the Creator, approves the Creator to spend RugInstance's tokens, withdraws an arbitrary token amount, or sends pool reserves. Calling RugToken.transfer from the Creator wallet cannot move tokens owned by RugInstance."],
      ["Unlock changes eligibility, not token custody", "founderUnlockTime is an immutable timestamp derived at construction. Reaching it does not execute a transfer, change status, reduce founderRemaining, approve an allowance, or credit the Creator. Before the timestamp, Creator-only rug reverts FounderLocked; after it, custody remains unchanged until a successful explicit transaction."],
      ["The one-shot sale routes tokens directly to RugPool", "A successful rug call requires the recorded Creator, Active status, elapsed lock, and a nonzero founderRemaining. It writes founderRemaining to zero, changes status to Rugged, transfers the full amount from RugInstance directly to the canonical RugPool, and calls sellFromRugInstance. The Creator receives net WBNB output, not the Founder Tokens."],
      ["Atomicity prevents a failed sale from stranding partial custody state", "The state writes, token transfer, pool reserve update, protocol-fee payment, Creator WBNB payment, and RugPulled event share one EVM transaction. If authorization, lock, deadline, slippage, token transfer, or pool execution reverts, all changes roll back. A failed attempt leaves the allocation in RugInstance and may be retried after correcting the failing condition."],
      ["The invariant uses a closed model and a deliberately zero Creator balance", "invariant_TokenIsConservedAndFounderNeverMovesToCreator sums balances across RugInstance, RugPool, Creator, treasury, handler, and three actors, then requires the sum to equal totalSupply and the modeled Creator balance to remain zero. The 12-action handler does not make the Creator an ordinary buyer, so 128 runs and 8,192 calls with zero handler reverts support the protocol-path claim—not a universal identity rule."],
      ["Immobility does not mean the Creator can never own or influence RugToken", "The Creator can independently buy fungible RugToken through an ordinary wallet or use related wallets or alternative pools; those units are not distinguishable on-chain as Founder Tokens. The invariant does not prove beneficial ownership, key security, fair distribution, price stability, liquidity, benign ordering, an exit, or recovery. It is bounded project-authored evidence, not an independent audit or safety finding. Organized mainnet activation remains NO-GO and total loss remains possible."],
    ],
  },
  "/what-does-protocol-fee-destination-mean": {
    eyebrow: "Builder desk · invariant 07 · protocol fee destination",
    title: "ONE WBNB FEE DESTINATION. TRACE EVERY MODELED ACCRUAL.",
    intro: "The protocol-fee destination property asks whether each modeled canonical swap fee reaches the immutable protocolTreasury in WBNB and whether the observed treasury increase equals the fees recorded by the harness. It does not mean swaps have only one fee, that the treasury cannot spend funds, or that every treasury receipt is necessarily a protocol fee.",
    sections: [
      ["Start with the exact quote asset and treasury", "The property concerns the canonical WBNB contract and the protocolTreasury address embedded in RugFactory, RugInstance, and RugPool. Native BNB, RugToken, another wrapped asset, an alternative pool, and a similarly labeled wallet are outside that accounting. Compare full addresses on the same chain and block before calculating any receipt."],
      ["The destination and fee rates are immutable for an existing deployment", "RugFactory stores protocolTreasury, swapFeeBps, protocolFeeBps, and creationFee as immutable configuration. Each successful RugInstance receives those values and constructs its RugPool with the same treasury and fee rates. The Factory owner can pause new creation but cannot rewrite the economics of an already created Rug or redirect that pool's protocol-fee destination."],
      ["Creation fee is a separate Factory transfer", "createRug pulls Creator stake plus creationFee into RugFactory, sends creationFee to protocolTreasury, deploys RugInstance, and transfers the stake to the instance. Deployed v0.4 uses a 0.003 WBNB creation fee. That payment is not part of the swap-fee invariant handler, which constructs RugInstance directly; Factory unit and scenario tests cover the creation split."],
      ["A buy calculates protocol fee from gross WBNB input", "For buyExactQuoteForTokens, protocolFeeQuote equals floor(quoteIn × protocolFeeBps ÷ 10,000). RugPool subtracts it from quoteIn before calculating poolQuoteIn, transfers the fee in WBNB to protocolTreasury, and retains the remaining pool input in reserves. With deployed v0.4, protocolFeeBps is 5 basis points."],
      ["The 25-basis-point pool fee is not a treasury transfer", "swapFeeBps is applied inside the constant-product amount-out formula. On a buy it reduces effective poolQuoteIn used for pricing; on a sell it reduces effective token input. The resulting value remains reflected in RugPool reserves rather than being separately transferred to protocolTreasury. Calling both components protocol fees would misstate their destinations."],
      ["A sell deducts protocol fee from gross WBNB output", "For sellExactTokensForQuote, RugPool calculates grossQuoteOut after the pool fee, then protocolFeeQuote as floor(grossQuoteOut × protocolFeeBps ÷ 10,000). The recipient receives gross output minus protocol fee and protocolTreasury receives the fee in WBNB. The two outgoing transfers sum to grossQuoteOut."],
      ["The Founder sale uses the same WBNB fee path", "RugInstance moves the full protocol-held Founder Allocation into RugPool and calls sellFromRugInstance. That path shares the internal sell accounting: Creator receives net WBNB output and protocolTreasury receives protocolFeeQuote. There is no separate Creator trading fee or Founder-token fee destination in MVP."],
      ["Rounding can make a valid fee equal zero", "RugMath.feeAmount uses integer floor division. A sufficiently small WBNB base amount can therefore produce zero protocolFeeQuote even when protocolFeeBps is nonzero. RugPool skips a zero-value treasury transfer and records zero in the Swap event; this is deterministic rounding, not evidence of a redirected fee."],
      ["The invariant compares observed balance deltas with an internal ledger", "After each successful modeled buy, sell, or rug, the handler measures protocolTreasury's WBNB balance increase and adds it to protocolFeesObserved. invariant_ProtocolFeesOnlyAccrueToTreasury requires treasury balance to equal that ledger. The reported property completed 128 runs and 8,192 targeted calls with zero handler reverts. The closed model does not include arbitrary third-party donations to treasury, Factory creation, treasury spending, fee-on-transfer WBNB, or alternative venues."],
      ["A correct destination is not a safety or stewardship verdict", "Atomic transaction rollback prevents a reverted canonical swap from leaving a partial fee transfer, but a successful receipt proves only that specified code moved WBNB as executed. The invariant does not audit treasury key custody, spending policy, ownership relationships, sanctions or tax treatment, price, liquidity, MEV, contract correctness, an exit, or recovery. It is bounded project-authored evidence, not an independent audit or endorsement. Organized mainnet activation remains NO-GO and total loss remains possible."],
    ],
  },
  "/what-does-non-decreasing-amm-k-mean": {
    eyebrow: "Builder desk · invariant 08 · non-decreasing AMM k",
    title: "K MAY GROW. CANONICAL SWAPS MUST NOT MAKE IT SHRINK.",
    intro: "For the canonical RugPool, k is the product of stored RugToken and WBNB reserves. The invariant records that product after modeled Active and Rugged actions and rejects any decrease. Because fees and integer rounding remain inside reserve arithmetic, the implementation targets non-decreasing—not byte-for-byte constant—k. This property says nothing by itself about price, depth, user value, or safety.",
    sections: [
      ["K uses stored reserves from one canonical pool", "Calculate k as uint256(reserveToken) × uint256(reserveQuote) from RugPool.getReserves at one state. Do not multiply wallet balances, D1 chart values, native BNB, an alternative-pool pair, or observations from different blocks. Stored reserves define canonical quote arithmetic; actual balances are checked separately by reserve reconciliation."],
      ["Successful finalization establishes the first product", "A Failed Opening creates no RugPool and therefore no canonical k. On success, RugInstance sends calculated RugToken and WBNB launch reserves into the new pool and initializes the stored uint112 pair once. That first nonzero product becomes the baseline after the handler records Active state."],
      ["A buy adds more quote than the pricing formula treats as effective input", "The buy path removes the WBNB protocol fee first, adds the remaining poolQuoteIn to stored quote reserve, and calculates tokensOut using poolQuoteIn after the 25-basis-point pool fee. Stored token reserve falls by tokensOut. Because the pricing formula treats less than the full pool input as effective, the post-swap reserve product should not fall."],
      ["A sell adds more tokens than the pricing formula treats as effective input", "The sell path adds full tokenIn to stored token reserve while grossQuoteOut is calculated from tokenIn after the pool fee. Stored quote reserve falls by grossQuoteOut; the separate WBNB protocol fee is carved from that gross output between recipient and treasury. The retained input effect and floor rounding should keep the stored product at least as large as before."],
      ["The Founder sale is another canonical sell for k accounting", "After unlock, RugInstance transfers the full founderRemaining amount directly into RugPool and calls sellFromRugInstance. That path uses the same internal sell formula and stored-reserve update as an ordinary token sale. A large input can move price and drain substantial WBNB while k still remains non-decreasing."],
      ["Integer floor rounding can increase k even without a visible fee transfer", "RugMath floors amountInAfterFee and amountOut. Output rounds down to whole token units, so the pool can retain a small residue compared with the continuous formula. A tiny protocol fee can also floor to zero. Non-decreasing k accepts equality or growth; it does not require a minimum increase per transaction."],
      ["Direct transfers do not update stored k", "Anyone can send RugToken or WBNB directly to the pool address. RugPool exposes no sync or skim function, so those units become actual-balance surplus and do not change reserveToken, reserveQuote, canonical quotes, or stored k. The dedicated donation unit test verifies this distinction; outside balance growth must not be presented as AMM invariant growth."],
      ["Reverts create no intermediate k checkpoint", "RugPool writes reserves before some token transfers, but the complete swap is one nonReentrant EVM transaction. An expired deadline, slippage failure, invalid recipient, transfer failure, or later revert rolls back reserve writes and payments. Only successful confirmed states belong in the k sequence."],
      ["The reported Foundry property also checks lifecycle status", "The handler records uint8 lifecycle status after targeted actions and sets statusDecreased if a later observation is numerically lower. In Active or Rugged it also records stored k and sets kDecreased if the next value is smaller. invariant_StatusAndConstantProductNeverDecrease requires both flags to remain false. The focused run completed 128 sequences and 8,192 targeted calls with zero handler reverts."],
      ["Non-decreasing k is not non-decreasing price, liquidity, or wealth", "A trade changes the reserve ratio, so token price can fall sharply while k grows. WBNB depth can shrink, slippage can rise, the Creator sale can cause major impact, related wallets and MEV can act, and alternative pools can diverge. The closed project-authored handler is bounded and excludes arbitrary tokens and external venues. This is not an independent audit, return guarantee, safety finding, or recovery promise. Organized mainnet activation remains NO-GO and total loss remains possible."],
    ],
  },
  "/what-does-status-monotonicity-mean": {
    eyebrow: "Builder desk · invariant 09 · lifecycle-status monotonicity",
    title: "STATUS MOVES FORWARD OR STAYS PUT. IT NEVER REOPENS.",
    intro: "A RugInstance starts in Opening and can finalize into one of two branches: Failed, or Active followed optionally by Rugged. Status monotonicity means successful calls never move its recorded lifecycle value backward. It is a state-machine property—not proof that progress is automatic, timely, profitable, or safe.",
    sections: [
      ["The enum order is part of the tested representation", "IRugInstance declares Opening, Failed, Active, and Rugged in that numeric order: 0, 1, 2, and 3. The invariant records the uint8 value and flags any later observation below the previous one. Numeric order makes backward movement detectable, but allowed transitions still come from explicit function checks rather than from the enum numbers alone."],
      ["Construction begins in Opening", "Solidity initializes the status storage slot to enum value zero, Opening. The constructor records immutable timing, economic, metadata, Creator, WBNB, and treasury configuration, but exposes no parameter that can create an instance directly as Failed, Active, or Rugged. RugToken and RugPool do not yet exist during this phase."],
      ["Contributions and clock movement do not advance status", "A successful contribution requires Opening and a timestamp before openingEnd; it updates per-wallet and total WBNB accounting without changing lifecycle status. Reaching openingEnd only closes contribution eligibility. Time passing by itself does not call finalize, deploy contracts, create refunds, or write a new status."],
      ["Finalize creates the only branch from Opening", "After openingEnd, any caller may invoke finalize once while status is Opening. If total contribution is below the immutable minimum, the call writes Failed and returns without creating a token or pool. Otherwise it calculates allocations, deploys RugToken and RugPool, writes Active, funds and initializes the pool, and emits LaunchSucceeded. A second finalize attempt rejects the non-Opening state."],
      ["Failed is terminal but still has settlement actions", "In Failed, each contributor may claim the recorded WBNB contribution once and the Creator may withdraw the recorded stake once. Those operations change claim or withdrawal flags and balances, not status. Failed cannot become Active or Rugged, and no cancellation or retry function can reopen the same launch; the Factory creation fee is outside the contributor refund."],
      ["Active persists through claims, swaps, and unlock", "Successful Opening claims, canonical buys, canonical sells, and reaching founderUnlockTime all leave status Active. Unlocking only makes the Creator's one-shot Founder Allocation sale eligible to be attempted; it does not execute the sale, create a deadline, or force lifecycle progress. An Active instance can therefore remain Active indefinitely."],
      ["The Founder sale is the only Active-to-Rugged transition", "rug() requires the immutable Creator, status Active, an expired Founder lock, and a nonzero founderRemaining amount. Before external token and pool calls it consumes founderRemaining and writes Rugged. The full allocation then enters the canonical RugPool for one sale. There is no partial-sale parameter and no route back to Active."],
      ["Rugged is terminal while selected actions continue", "Rugged does not freeze RugToken or RugPool. Unclaimed successful-Opening entitlements may still be claimed, and canonical buying and selling remain available subject to ordinary execution conditions. Those calls do not alter lifecycle status. A second protocol Founder sale rejects Rugged with BadStatus even though ordinary wallet-held tokens remain transferable."],
      ["A reverted transition creates no intermediate status", "Finalize and rug are nonReentrant EVM transactions. If a later deployment, transfer, pool initialization, deadline, slippage, or token operation reverts, earlier status writes and asset movements roll back atomically. Only a successful confirmed transaction establishes a new lifecycle checkpoint; pending calldata, simulations, and failed receipts do not."],
      ["The invariant is bounded evidence, not liveness or safety proof", "The 12-action handler calls _recordState after modeled lifecycle and market actions, sets statusDecreased if currentStatus is below lastStatus, and invariant_StatusAndConstantProductNeverDecrease requires that flag to remain false. The focused reported run completed 128 sequences and 8,192 targeted calls with zero handler reverts. It does not prove someone will finalize, claim, trade, or rug; guarantee timing, price, liquidity, recovery, or key security; cover arbitrary integrations; or replace an independent audit. Organized mainnet activation remains NO-GO and total loss remains possible."],
    ],
  },
  "/what-does-economic-allocation-consistency-mean": {
    eyebrow: "Builder desk · invariant 10 · economic-allocation consistency",
    title: "THREE RECORDED ALLOCATIONS. ONE FIXED SUPPLY. KEEP THE EQUATION HONEST.",
    intro: "On a successful launch, Rugspull records the Founder Allocation, aggregate Opening allocation, and initial canonical-pool token reserve as a partition of fixed RugToken supply. The invariant checks that those launch fields remain algebraically consistent before and after the one-shot Founder sale. It does not claim that the fields are live balances or current market reserves.",
    sections: [
      ["Begin with one RugInstance and its immutable supply", "Let T be tokenTotalSupply and f be founderBps for the exact RugInstance configuration. RugMath computes F = floor(T × f ÷ 10,000). The remaining non-Founder supply is N = T − F. The property is scoped to those recorded values for one instance; it is not an allocation rule across similarly named tokens, alternative deployments, wallets, or pools."],
      ["Failed launches never activate the partition", "During Opening there is no RugToken, pool, or finalized allocation. If total contribution is below minLaunchAmount, finalize writes Failed and returns before calculating or storing F, A, or X and before deploying RugToken. The invariant therefore returns early outside Active and Rugged; Failed refund accounting is covered by separate conservation and scenario tests."],
      ["Accepted contribution is capped before token allocation", "For a successful launch, Q = min(totalContributed, openingCap). WBNB above Q remains part of proportional excess-refund accounting rather than purchasing more Opening tokens. Creator stake C and accepted contribution Q set the initial WBNB reserve Y = C + Q; this property focuses on the RugToken-side allocation fields, not the WBNB equation."],
      ["Opening allocation and pool allocation divide non-Founder supply", "RugInstance calculates A = floor(N × Q ÷ (C + 2Q)) for the aggregate Opening allocation, then X = N − A for the initial canonical-pool token reserve. Because X is defined by subtraction after A is rounded, A + X equals N exactly in integer arithmetic. The floor can affect the split but cannot create or remove units from this recorded partition."],
      ["Active preserves F + A + X = T", "On successful finalization, RugInstance stores founderRemaining = F, openingTokenAllocation = A, and poolTokenReserve = X, then mints exactly T to itself and transfers X into RugPool. While status is Active, invariant_EconomicAllocationsRemainConsistent requires the three recorded fields to sum to tokenTotalSupply and separately requires founderRemaining to equal the fixture's expected Founder amount."],
      ["These are launch records, not three live balances", "openingTokenAllocation remains the original aggregate entitlement after wallets claim; it is not decremented per claim. poolTokenReserve remains the initial X after buys, sells, donations, and the Founder sale; current RugPool reserves come from getReserves. founderRemaining is the only one of the three fields consumed later. Use token balances and reserve reconciliation for current custody, not this launch equation."],
      ["Claims and swaps do not rewrite the allocation formula", "A successful claim transfers floor(A × walletContribution ÷ totalContributed) from RugInstance, and canonical swaps move tokens between RugPool and traders. Those actions can change balances and leave rounding residue, but they do not change A or X. This preserves the reproducible launch calculation while separate conservation checks account for where actual tokens moved."],
      ["The one-shot Founder sale consumes only founderRemaining", "rug() reads the full F from founderRemaining, writes that field to zero, and sends the tokens directly into the canonical pool for one sale. A successful transaction advances status to Rugged. A revert rolls the zeroing, transfer, swap, fees, and status change back atomically, so a failed attempt leaves the Active equation intact."],
      ["Rugged preserves A + X = T − F, not a live custody total", "After the successful Founder sale, the invariant expects founderRemaining = 0 and compares founderRemaining + A + X with T minus the original F. The sold Founder Tokens still exist and may now sit in RugPool or later wallets; subtracting F does not mean they were burned. It means the protocol-held allocation right was consumed and is no longer represented by founderRemaining."],
      ["The reported fixture and property have explicit limits", "The invariant handler uses T = 1,000,000 tokens and founderBps = 4,500, so its Rugged-side expectation hard-codes F = 450,000 tokens for that fixture. Production values must be derived from each instance's immutable configuration rather than copied from the test literal. The reported property completed 128 runs and 8,192 targeted calls with zero handler reverts, but bounded project-authored tests do not prove live balances, price, liquidity, fair ordering, recovery, exhaustive correctness, or safety. Independent audit and organized mainnet activation remain pending; total loss remains possible."],
    ],
  },
  "/why-opening-price-is-not-below-initial-pool-price": {
    eyebrow: "Economics desk · Opening and initial-pool price relation",
    title: "OPENING PRICE MEETS THE POOL PRICE. FLOOR ROUNDING CAN ONLY LIFT IT.",
    intro: "Rugspull calculates the aggregate Opening allocation and the initial canonical-pool reserves from the same accepted WBNB amount. Without integer rounding, their implied token prices are equal. Because the Opening token allocation is rounded down, the implemented Opening price can be slightly higher—but not lower—than the initial pool spot price. This is a launch-formula property, not a trading or profit guarantee.",
    sections: [
      ["Compare one successful RugInstance at finalization", "Use the exact instance's immutable token supply, Founder basis points, Creator stake, Opening cap, total contribution, and finalized allocation fields. The comparison concerns the aggregate Opening price and the canonical RugPool's initial reserve ratio at successful finalization. It does not compare later swaps, alternative pools, a D1 chart, native BNB, or values sampled from different blocks."],
      ["A Failed Opening has no pool price to compare", "If total contribution U is below minLaunchAmount, finalize writes Failed and creates neither RugToken nor RugPool. Contributors can claim recorded WBNB refunds and the Creator can withdraw the stake, but there is no A, X, Y, or canonical launch spot price. The inequality applies only to the successful branch."],
      ["Use accepted contribution Q, not gross contribution U", "The contract sets Q = min(U, openingCap). Contribution above the cap participates in proportional excess-refund accounting and does not enlarge the accepted WBNB used for launch pricing. Substituting U for Q in the price formula during oversubscription produces the wrong result."],
      ["Remove Founder Allocation before splitting launch tokens", "Let T be fixed RugToken supply, F = floor(T × founderBps ÷ 10,000), and N = T − F. Only N is split between the aggregate Opening allocation and initial pool token reserve. The Founder Allocation stays in RugInstance until the later one-shot sale and is not part of either side of this initial price comparison."],
      ["The three launch formulas share C and Q", "With Creator stake C, RugMath calculates A = floor(N × Q ÷ (C + 2Q)), then X = N − A and Y = C + Q. A is the aggregate Opening token allocation, X is the canonical pool's initial RugToken reserve, and Y is its initial WBNB reserve. These stored launch values—not current balances after trading—define the comparison."],
      ["Without floor rounding, both implied prices are equal", "Treat A as the exact fraction NQ ÷ (C + 2Q). The Opening WBNB-per-token price is Q ÷ A, which simplifies to (C + 2Q) ÷ N. Because X then equals N(C + Q) ÷ (C + 2Q), the initial pool spot ratio Y ÷ X simplifies to the same expression. This equality is algebra, not an oracle quote."],
      ["Flooring A moves the inequality in one direction", "Solidity integer division rounds A down to whole token units. A smaller denominator makes Q ÷ A no lower, while X = N − A becomes no smaller and makes Y ÷ X no higher. Therefore floor rounding can create a small premium for Opening tokens relative to the initial pool ratio, but cannot create the prohibited Opening discount under the stated inputs."],
      ["The test uses cross multiplication, not floating point", "The implementation test checks Q × X ≥ Y × A, which is equivalent to Q ÷ A ≥ Y ÷ X when A and X are positive. Cross multiplication avoids floating-point arithmetic and preserves exact integer comparison. Verification should derive all four values from the same finalized instance and guard against mixing token decimals, WBNB decimals, or stale records."],
      ["Foundry explores bounded formula inputs", "testFuzzOpeningPriceNotBelowPool bounds Creator stake, contribution, and cap from 10^12 to 10^24 WBNB units, Founder basis points from 1,000 to 7,000, and uses a fixed one-billion-token supply. It checks supply partition, positive A and X, and the cross-multiplied inequality across 256 generated cases in the reported run. These bounds and one formula implementation are meaningful evidence, not exhaustive mathematical or deployment proof."],
      ["A launch-price relation is not an execution guarantee", "The initial spot ratio has no depth or slippage promise. Once trading begins, buys, sells, fees, MEV, transaction ordering, the Founder sale, related wallets, direct transfers, and alternative pools can change executable outcomes. Opening claims require a separate transaction and may include integer residue; a buyer, exit, recovery, or profit is not guaranteed. Project-authored tests are not an independent audit or safety certification. Organized mainnet activation remains NO-GO and total loss remains possible."],
    ],
  },
  "/why-pro-rata-claims-can-leave-rounding-residue": {
    eyebrow: "Economics desk · per-wallet claim arithmetic",
    title: "EVERY WALLET ROUNDS DOWN. THE TOTAL NEVER ROUNDS UP.",
    intro: "A successful Opening divides one aggregate RugToken allocation and any excess WBNB among contributing addresses. Solidity calculates each address separately with integer division, so every result rounds down. The combined claims cannot exceed either aggregate amount, but small token or WBNB residue can remain in RugInstance.",
    sections: [
      ["Define one successful Opening", "Let A be openingTokenAllocation, U be totalContributed, Q be acceptedContribution, and E = U − Q be aggregate excess WBNB. For one contributing address i, let u_i be its recorded contribution. These values come from the same finalized RugInstance; mixing estimates, another instance, native BNB, or later market balances produces the wrong claim calculation."],
      ["Failed uses exact recorded refunds instead", "This proportional claim arithmetic applies only after successful finalization. If U is below minLaunchAmount, status becomes Failed, no RugToken or RugPool is created, and claimFailedRefund() returns that caller's full recorded contribution u_i. It does not use A, Q, E, or a proportional excess-refund formula."],
      ["The address owns a fraction of the batch", "For the successful branch, each contributor's share is u_i ÷ U. claimOpening() reads contributionOf[msg.sender], so the entitlement belongs to the original contributing address. The Creator is prohibited from contributing, and the frontend, Worker, D1, operator, or another wallet cannot merge, redirect, or claim that address's share."],
      ["Opening tokens round down per address", "The token output is token_i = floor(A × u_i ÷ U). Solidity performs multiplication and integer division in token base units; any fractional remainder below one unit is discarded for that address. A very small calculated entitlement can therefore become zero even though the address has a nonzero contribution."],
      ["Excess WBNB also rounds down per address", "The refund output is refund_i = floor(E × u_i ÷ U), where E = U − Q. If U is at or below the cap, E is zero and every successful-Opening excess refund is zero. When oversubscribed, the same final contribution denominator allocates the excess in WBNB wei."],
      ["The aggregate cannot overpay", "Because every term is floored independently, sum_i floor(A × u_i ÷ U) ≤ A and sum_i floor(E × u_i ÷ U) ≤ E when the included contributions sum to U. Claim order does not change these formulas. The inequalities prevent the listed wallet outputs from collectively rounding above either aggregate allocation."],
      ["Residue is not another claim right", "Strict inequality is possible: per-address fractional parts can add up to whole base units after each was already discarded. Those token units or WBNB wei remain in RugInstance. The current contract does not reallocate them to the final claimant, increase a later claim, provide a support override, or expose an automatic residue sweep or refund function."],
      ["Address splitting can change rounding, not the ceiling", "Because flooring happens per address, dividing the same economic contribution among several addresses can change the sum of their rounded outputs compared with using one address. The contract does not solve identity or related-wallet behavior. Even so, for the recorded address set, aggregate paid tokens cannot exceed A and aggregate excess refunds cannot exceed E."],
      ["Each successful claim is one-shot and atomic", "claimOpening() is available in Active and remains available in Rugged. It checks contribution and claimed state, records claimed before external transfers, sends any nonzero token and WBNB outputs, and emits both amounts. A second successful call reverts; if a required transfer reverts, EVM atomicity rolls back the claimed update and transfers together."],
      ["The fuzz property has explicit limits", "testFuzzClaimSumsDoNotExceedAllocations models three addresses, bounds each contribution from 1 to 10^24 units, allocation from 1 to 10^27 units, and accepted contribution from zero through U. The reported focused run checks both aggregate inequalities across 256 generated cases. This bounded project-authored test does not prove complete settlement, live balances, identity fairness, price, liquidity, recovery, exhaustive correctness, audit, or safety. Organized mainnet activation remains NO-GO and total loss remains possible."],
    ],
  },
  "/what-does-settled-mean-for-claims-and-refunds": {
    eyebrow: "Evidence desk · claim and refund reporting",
    title: "SETTLED MEANS EVERY RIGHT WAS EXERCISED. IT DOES NOT MEAN ZERO BALANCE.",
    intro: "Rugspull uses settled as a reporting label for completed claim or refund rights, not as a smart-contract lifecycle status. The label is appropriate only after every eligible address-specific action—and, for Failed, the separate Creator-stake withdrawal—is evidenced. It does not require RugInstance to hold zero assets and does not describe trading, Founder behavior, or safety.",
    sections: [
      ["Settled is not an on-chain status", "RugInstance exposes Opening, Failed, Active, and Rugged. It has no Settled enum value, settle() function, aggregate claims-complete flag, or contract-side contributor count. A publication that uses settled is making a dated evidence statement and must state whether it means successful-Opening claims or Failed refunds rather than inventing a fifth lifecycle state."],
      ["Start with the complete eligible address set", "Each successful contribute() call emits Contributed and adds to contributionOf[user], but the contract stores no contributor array. To assess completion, reconstruct distinct contributing addresses from the canonical RugInstance's event history, aggregate repeated contributions by address, and reconcile the set and amounts with totalContributed and direct contributionOf reads. A partial API list, screenshot, support roster, or one known wallet is not enough."],
      ["Successful-Opening settlement means every contributor claimed", "After Active—and still after Rugged—each address with a positive recorded contribution calls claimOpening() once. Completion evidence is a successful transaction plus claimed[address] = true and the matching ClaimedOpening event with tokenAmount and refundAmount. The accepted WBNB already moved into RugPool at finalization; Creator stake is pool reserve on this branch and is not a separate refundable right."],
      ["Failed contributor settlement uses full refunds", "After Failed, each contributing address calls claimFailedRefund() for its full recorded contribution. A successful receipt, claimed[address] = true, and ClaimedFailedRefund amount should reconcile with that address's contributionOf value. There is no RugToken, RugPool, successful-Opening token allocation, or proportional excess-refund calculation on this branch."],
      ["Failed also has a separate Creator right", "Contributor refunds alone do not complete the Failed settlement record. The recorded Creator must call withdrawCreatorStakeAfterFailure(), after which creatorStakeWithdrawn is true and CreatorStakeWithdrawn records the Creator and amount. The creation fee was already sent to treasury, while gas is native BNB spent by transaction senders; neither is an outstanding Failed refund."],
      ["A reverted transaction is still outstanding", "Both contributor paths write claimed before transferring, and the Creator path writes creatorStakeWithdrawn before transferring. If a later transfer reverts, EVM atomicity rolls back the flag, assets, and event. Count only successful receipts whose current flags and balance changes agree. A submitted hash, pending transaction, failed receipt, disabled button, or support confirmation is not completion."],
      ["Publish eligible, completed, and outstanding separately", "For each branch, report the cutoff block and time, eligible address count and amount, completed address count and amount, and outstanding address count and amount. On Failed, report Creator stake eligible, withdrawn, and outstanding separately. Do not collapse contributor and Creator rights into one percentage or infer universal completion from one example transaction."],
      ["Zero RugInstance balance is neither required nor sufficient", "Successful per-wallet floor division can leave RugToken units or WBNB wei in RugInstance after every contributor claims. Those units are not another claimant's entitlement and the current contract does not automatically sweep them. Conversely, a low or zero observed balance without the complete address, receipt, event, and flag reconciliation does not prove that every eligible right was correctly executed."],
      ["Claim settlement does not settle the whole Rug", "All contributor claims may be complete while status remains Active, the Founder Allocation remains held, and trading continues. They may also complete after status becomes Rugged. Founder unlock or the one-shot Founder sale, swaps, protocol fees, alternative pools, market losses, key access, and later chain activity are separate facts. Use a scoped phrase such as successful-Opening claims settled at block N, not Rug settled."],
      ["The evidence label has strict limits", "Project tests cover one-shot successful claims, Failed refunds, Creator withdrawal, atomic rollback, and lifecycle scenarios, but the contract does not enumerate every real contributor for a reporter. A settlement record therefore depends on complete event reconstruction and current reads at a named cutoff. Even a correct record does not prove audit, safety, identity fairness, liquidity, price, recovery, support quality, or absence of future losses. Organized mainnet activation remains NO-GO and total loss remains possible."],
    ],
  },
  "/how-to-verify-claims-and-refunds-on-bscscan": {
    eyebrow: "BscScan field desk · claim and refund evidence",
    title: "FOLLOW EVENTS. READ FLAGS. RECONCILE EVERY CLAIM.",
    intro: "A transaction hash alone does not prove that every Rugspull claim or refund is complete. Start from the canonical RugInstance, reconstruct every contributing address from events, inspect successful receipts, read current one-shot flags, and reconcile the amounts at one named BNB Chain block and time.",
    sections: [
      ["Begin with the canonical RugInstance", "Use the Rug address emitted by the canonical RugFactory and confirm the network before reading anything else. BNB Smart Chain mainnet uses chain id 56; the published demonstrations use BSC Testnet chain id 97. A same-looking address, token page, search result, support message, or alternative deployment is not interchangeable with the selected RugInstance."],
      ["Record one cutoff block and time", "Explorer pages can update while evidence is being assembled. Write down the latest block included in the review, its timestamp, the RugInstance address, network, and the transaction hashes used. Eligible, completed, and outstanding counts must all use that same cutoff; mixing a newer claimed flag with an older event export creates a false reconciliation."],
      ["Reconstruct contributors from Contributed events", "Open the RugInstance event log and collect every successful Contributed event through the cutoff. Each record identifies the indexed Rug and user plus the WBNB amount. Deduplicate users, but sum repeated events for the same address. The contract stores contributionOf by address and no iterable contributor array, so one screenshot or the latest event is not a complete address set."],
      ["Reconcile the eligible address set", "For every distinct contributor, read contributionOf(address) and compare it with that address's summed Contributed amounts. Then sum the distinct-address totals and compare the result with totalContributed. Reverted contribution transactions emit no surviving event and change no state; native BNB transfers or unrelated WBNB transfers do not become contributionOf entries."],
      ["Determine the finalization branch first", "Read status and the finalization receipt. LaunchFailed identifies the Failed branch and records total versus minimum. LaunchSucceeded identifies the successful branch, Token and RugPool, accepted contribution, Opening allocation, initial reserves, and Founder allocation. Opening has no claim path yet; Active and Rugged share the successful-Opening claim path."],
      ["Verify each successful-Opening claim", "For every eligible address after Active or Rugged, find a successful claimOpening receipt and its ClaimedOpening event. Confirm the event user, tokenAmount, and refundAmount, then read claimed(address) = true. Recalculate floor(openingTokenAllocation × contribution ÷ totalContributed) and any floor((totalContributed − acceptedContribution) × contribution ÷ totalContributed) excess-WBNB refund from the same instance values."],
      ["Verify each Failed contributor refund", "For a Failed instance, find each successful claimFailedRefund receipt and ClaimedFailedRefund event. The event amount must equal that address's full contributionOf value, and claimed(address) must be true. Failed creates no RugToken or RugPool and does not use the successful-Opening pro-rata token or excess-refund formulas."],
      ["Verify the separate Creator withdrawal", "Failed contributor completion is not the Creator-stake record. Separately confirm a successful withdrawCreatorStakeAfterFailure receipt from the recorded creator, the CreatorStakeWithdrawn event, creatorStakeWithdrawn = true, and the immutable creatorStake amount. The creation fee already sent to treasury and native BNB gas are not Failed refund rights."],
      ["Treat failed and pending transactions as outstanding", "A pending hash is not a receipt, and a reverted receipt is not a completed claim. EVM atomicity removes the flag update, transfers, and event when a later operation reverts. Count completion only when the successful receipt, matching event, current claimed or Creator-withdrawn flag, and expected amount agree; explorer labels, UI buttons, or support statements are secondary aids."],
      ["Publish the limits with the reconciliation", "Report eligible, completed, and outstanding addresses and amounts separately, plus the cutoff and any rounding residue. Do not infer completion from a zero balance or infer incompletion solely from a nonzero balance. Explorer availability, verified source, project-authored tests, and a correct reconciliation do not prove audit, safety, identity fairness, liquidity, recovery, or future execution. Organized mainnet activation remains NO-GO and total loss remains possible."],
    ],
  },
  "/what-is-a-transaction-receipt-on-bnb-chain": {
    eyebrow: "BNB Chain field guide · execution evidence",
    title: "A HASH MEANS SUBMITTED. A SUCCESSFUL RECEIPT MEANS EXECUTED.",
    intro: "A BNB Chain transaction hash identifies a submitted transaction. The receipt appears only after inclusion and records whether execution succeeded, which block included it, how much gas was used, and which logs survived. It is essential evidence, but it must still be matched to the correct network, contract, decoded action, transfers, and current state.",
    sections: [
      ["Submission is not execution", "A wallet can produce a transaction hash before the transaction is mined. While pending, ordering, gas pricing, nonce conflicts, replacement, RPC propagation, and other chain conditions can still change the outcome. A pending explorer page, wallet success toast, copied hash, or signed payload does not prove that contract state changed."],
      ["The receipt anchors one included transaction", "After inclusion, the receipt identifies the transaction hash, block hash and number, transaction index, sender, destination or created contract, gas used, effective gas price where exposed, cumulative gas, status, and logs. Record the network and a stable cutoff with it; the same-looking address or hash on another chain is not the same execution record."],
      ["Status separates success from revert", "A successful status means the EVM call completed without reverting in that included transaction. A failed status means state writes, token transfers, WBNB transfers, child calls, and emitted logs from the reverted execution were rolled back atomically. The sender can still spend native BNB on gas for a failed transaction."],
      ["Gas is not the transferred asset", "BNB Chain transaction gas is paid in native BNB by the sender. Rugspull settlement and canonical trading use WBNB, an ERC-20 token with separate balances, approvals, and Transfer logs. Do not count gas as Creator stake, contribution, pool reserve, swap input, claim refund, or protocol fee."],
      ["From, to, and input identify the attempted call", "Confirm that from is the expected wallet and to is the exact RugFactory, RugInstance, RugToken, RugPool, or WBNB contract. Decode the function selector and arguments against the verified contract ABI or the exact source revision. An explorer method label is convenient, but an address, ABI, proxy assumption, or network mismatch can make a friendly label misleading."],
      ["Logs are surviving contract statements", "Successful execution may emit events such as RugCreated, Contributed, LaunchSucceeded, LaunchFailed, ClaimedOpening, ClaimedFailedRefund, CreatorStakeWithdrawn, Swap, or RugPulled. Each log has an emitting contract address plus topics and data. Decode against that emitter's exact event signature, and never treat a similarly named event from an unrelated contract as canonical evidence."],
      ["Transfer logs show asset movement, not business meaning by themselves", "ERC-20 Transfer logs help trace RugToken and WBNB movement, but the same transfer shape can represent a contribution, reserve deposit, claim, refund, swap, fee, donation, or unrelated transfer. Reconcile token address, sender, recipient, amount, called function, protocol event, and post-transaction balances before assigning meaning."],
      ["Current reads complete the evidence", "A receipt proves what happened in one included transaction, not the current value of every related field. Read status, contributionOf, claimed, creatorStakeWithdrawn, founderRemaining, Token and Pool addresses, stored reserves, and actual balances at the named cutoff. Later successful transactions can legitimately change current state without changing the historical receipt."],
      ["Simulation and confirmation answer different questions", "A wallet estimate, eth_call, gas estimate, or explorer simulation predicts execution against a chosen state without committing it. A successful receipt records actual included execution. Even then, an operator should use a sufficiently stable cutoff and consistent RPC or explorer evidence rather than present a just-seen pending or unstable observation as final reporting."],
  ...
  "/office-counter": {
    eyebrow: "Office Counter · report 025",
    title: "EVIDENCE, NOT GROWTH THEATER.",
    intro: "A dated public snapshot of what is deployed, what was tested, what was published, and what remains unresolved. Counts below use a 2026-07-19 23:13 CST cutoff and are not live analytics.",
    sections: [
      ...
      ["Contract test record", "Foundry reran 41 tests across unit, fuzz, invariant, and scenario families on 2026-07-19: 41 passed and zero failed. The seven invariant properties each completed 128 runs and 8,192 calls with zero handler reverts across 12 targeted selectors. Passing project-authored tests are bounded evidence, not exhaustive proof, an independent audit, or a safety finding."],
      ...
      ["Public evidence inventory", "53 evergreen mechanism, security, API-reference, education, WBNB, token-approval, slippage, transaction-receipt, constant-product AMM, liquidity, BscScan reserve-verification, claim/refund-verification, MEV, alternative-pool, smart-contract-invariant, token-conservation, WBNB-conservation, no-double-claim, no-double-rug, reserve-reconciliation, Founder-Token-immobility, protocol-fee-destination, non-decreasing-AMM-k, lifecycle-status-monotonicity, economic-allocation-consistency, Opening-versus-initial-pool-price, pro-rata-claim-rounding, claim-and-refund-settlement, verified-source/audit-boundary, D1/financial-truth, Opening, Creator-stake, contribution-identity, cancellation, excess-refund, permissionless-finalization, successful-claim, Failed-refund, Founder-sale, Still-Waiting, post-Rugged trading, lifecycle-template, Creator-readback, community-safety, and gate-review pages plus the TESTNET lifecycle archive and this dated Office Counter make 55 trust-first evidence routes public. The sitemap contains 58 URLs. Google Search Console accepted the refreshed sitemap on 2026-07-19 and its aggregate row reports 58 discovered pages with 0 videos. URL Inspection classified the new BNB Chain transaction-receipt guide as not indexed because Google could not recognize the URL and detected no referring sitemap or referring page; a priority indexing request was initiated. The overview reports 0 web-search clicks. Discovery and notification requests do not prove indexing, ranking, clicks, visits, or use."],
      ...
      ["Review and directory state", "DappBay's latest owner-side My Project route rendered Page Not Found, its submit page requested a fresh wallet connection, and its public search returned No related dApps or campaigns; the earlier Security Reviewing state is not promoted to a current result. RootData's prior dashboard session expired, so its last authenticated Pending Review state was not promoted either. DappRadar's official submit link and Developers route redirect to its homepage, which exposes no submit entry. Focused public searches found no Rugspull detail page on those services, MathWallet, Magic Store, or Moralis Web3 Wiki. BNB Chain Awesome PR #13 and Electric Capital PR #2932 remain open and clean. GitHub Issue 1 now has six project-authored review comments and no external response; the newest publicly requests proof or a counterexample for a candidate pro-rata rounding-residue bound. None of these states is an audit, listing approval, independent review, recommendation, partnership, or BNB Chain endorsement."],
      ["Deployment continuity record", "A prior README-only GitHub push exposed source drift and temporarily caused sampled edges to disagree. The deployment source was synchronized, and subsequent GitHub-triggered deployments restored consistent public assets. The current production sitemap serves 58 unique URLs, and the BNB Chain transaction-receipt guide from commit 4785c14 renders with its canonical metadata, ten evidence steps, structured data, and risk boundaries. This is a dated recovery and deployment observation, not an uptime SLA or future-availability promise."],
      ["Machine-readable discovery", "Mirrored root and well-known security.txt files, llms.txt, a ten-item RSS evidence feed, homepage RSS discovery, the IndexNow key, and a source-controlled IndexNow submission command are public. The feed and llms.txt link the WBNB, token-approval, slippage, transaction-receipt, constant-product AMM, liquidity, reserve-verification, claim/refund-verification, MEV, alternative-pool, smart-contract-invariant, token-conservation, WBNB-conservation, no-double-claim, no-double-rug, reserve-reconciliation, Founder-Token-immobility, protocol-fee-destination, non-decreasing-AMM-k, lifecycle-status-monotonicity, economic-allocation-consistency, Opening-versus-initial-pool-price, pro-rata-claim-rounding, claim-and-refund-settlement, verified-source/audit-boundary, and D1/financial-truth guides directly. IndexNow accepted 69 URLs: 58 sitemap URLs plus 11 machine-readable resources. Security contact, pending independent audit, total loss, organized-mainnet NO-GO, and no-inferred-endorsement boundaries are explicit. Publication and successful notification do not prove crawl, subscription, AI ingestion, response, indexing, or readership."],
