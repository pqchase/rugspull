...
The protocol uses Solidity/Foundry contracts, WBNB as the only quote asset, an opening batch with unified pricing, failed-launch refunds, and an internal canonical constant-product AMM. Native BNB remains necessary for transaction gas; WBNB balances and allowances do not pay gas. Founder allocation never leaves `RugInstance` and can only be sold once into the canonical `RugPool`.
